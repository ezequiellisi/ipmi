...
void draw() {
  ...
  if (segundos<=9) {
    ...
    //Le doy movimiento vertical ("hacia arriba") al primer texto
    if (posY>10) {
      posY = posY - 0.5;
    } else {
      posY=10;
    ...
    //Reinicio los valores de posX y posY
    if (segundos == 10) {
      posX = width;
      posY = 120;
    }
    ...
    // le doy movimiento horizontal "hacia la izquierda".
    if (posX > 80) {
      posX -= 2;
    } else {
      posX = 80;
    ...
    //Reinicio la ubicación de posX y de posY
    if (segundos == 19) {
      posX=100;
      posY=480;
    }
    ...
    //Le doy movimiento vertical ("hacia arriba") al tercer texto
    if (posY>120) {
      posY = posY - 1;
    } else {
      posY=120;
...
void mousePressed() {
  if (mouseX>500 && mouseX<600 && mouseY>400 && mouseY<450 && segundos>=30) {
    segundos = 0;
    posX = 20;
    posY = 480;
  }
